...
netbox_database_maintenance_db: postgres
...
If the database is already created but user does not have rights to connect to 
default maintenance db just specify target database as maintenance.
...
- hosts: netbox.idolactiviti.es
  ...
  vars:
    ...
    netbox_database_maintenance_db: netbox_prod
